...
# =============================================================================
#  DETECCIÓN DE ROL POR HARDWARE (ACTUALIZADO)
#  ─────────────────────────────
...
#  MISO → GP4
# =============================================================================
#  PINOUT PERIFÉRICOS ACTUALIZADO
#  ──────────────────
...
PAYLOAD_SIZE     = 16       # Bytes por paquete (actualizado)
RF_REINTENTOS    = 5        
RF_REINTENTO_MS  = 15       
HEARTBEAT_MS     = 600      
...
S_ERROR     = const(10)
M_IDLE       = const(11)
M_PREPARADO  = const(12)
M_ARMADO     = const(13)
M_REGISTRADO = const(14)
M_ERROR      = const(15)
...
def init_radio():
    """
    Inicialización de SPI usando el bloque de manejo de errores específico.
    """
    Chip_Enable = 6
    Chip_Select = 5
    ...
    try:
        ...
        try:
            ...
            pass # Si falla al desinicializar, ignoramos y seguimos
        utime.sleep_ms(500)
...
def enviar_robusto(radio, addr, msg_id, valor=0, intentos=RF_REINTENTOS, pausa_ms=RF_REINTENTO_MS):
    ...
    try:
        radio.set_tx_address(addr)
    except Exception as e:
        print("[RF] Error set_tx_address: {}".format(e))
        return False
...
def run_maestro():
    ...
    radio = init_radio()
    radio.set_rx_address(1, ADDR_MAESTRO)
    radio.set_mode_rx()
    ...
    print("[MAESTRO] Sistema listo. Esperando BTN_START (GP1)...")
    ...
    while True:
        try:
            ...
            if btn_start.value() == 0 and estado == S_IDLE:
...
def run_salida():
    ...
    radio = init_radio()
    radio.set_rx_address(1, ADDR_SALIDA)
    radio.set_mode_rx()
    ...
    def habilitar_sensor():
        try: sensor.irq(trigger=Pin.IRQ_FALLING, handler=isr_sensor)
...
def run_meta():
    ...
    sensor_meta = Pin(1,  Pin.IN,  Pin.PULL_UP)
    led_error   = Pin(7,  Pin.OUT, value=0)

    radio = init_radio()
    radio.set_rx_address(1, ADDR_META)
    radio.set_mode_rx()
    ...
    def habilitar_sensor():
        try: sensor_meta.irq(trigger=Pin.IRQ_FALLING, handler=isr_meta)
